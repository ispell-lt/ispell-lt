...
## kai kas permetama á gilesnius katalogus (fiziniø target'ø paieðka)
vpath % $(D_BUILD)
vpath % $(D_BUILD)/$(D_MYSPELL)


## dinamiðkai suformuojami/keièiami, tad nereikëtø iðoriðkai nustatyti
override D_DST   =
override D_DST_T =
...
## Nors GnuWin turi dos2unix, taèiau gana archaiðkà versijà v0.9, kuri 
## kreivai veikia su temp failais. stdio.h:P_tmpdir reikðmë (turbût /tmp) 
## neteisingai interpretuojama ir tmpnam() graþina failø pavadinimus root'e.
## (jei ateityje GnuWin atnaujins versijà, tai vertëtø naudotis ðia programa)
dos2unix = tr -d '\r' < $1 > $1.new; mv -f $1.new $1

## kiek saugesnio ðalinimo komanda; neiððokama aukðèiau esamojo katalogo
...
## Aspell, pavyzdþiui, sutrinka skaitydamas config ir turbût kt. failus su
## \r\n, o ávairûs (python, perl) script'ai, native aplinkoje, þinoma, iðveda
## \r\n. Na ir kiti suderinamumo aspektai. 
## (tai OS/ENV check kodas neiðvengiamas...)
...
ifdef MSWIN
    ifndef SHELL_SH
        ## reiðkia PATH nerado sh.exe; dar yra ðansø, kad yra koks *sh
    ...
    ifdef GNUWIN
        ## atskirus langus iðmeta gal tik win-zsh; nepakenks, jei script'ai 
        ## bus kvieèiami su iðskirtinai nurodytu interpretatoriumi
        PYCMD = python
        PLCMD = perl
    endif

    ## Kai kà, pvz. find gali pasigauti ið system32; reikia nurodyti visà 
    ## kelià. Kabutës bûtinos, nes GnuWin programos graþina ne posix path 
...
##############################################
## Þodynø (ispell/myspell/aspell) target'ai ##
##############################################
...
# ketindamas nustatyti teises ir kuriamiems katalogams kaþkodël nustato 0444;
# (vëliau rm -rf negali paðalinti tokiø katalogø)
